...
# Confirm Debian 12 as a base
source /etc/os-release

if [ "$ID" != "debian" -a "$VERSION_ID" != "12" ]; then
    echo "################################# "
    echo "############ WARNING ############ "
    echo "################################# "
    echo
    echo "This script only works on Debian 12!"
    echo "You're using: ${NAME} ${VERSION}"
    echo "Better ABORT with Ctrl+C. Or press any key to continue the install"
    read
else
    echo "Passed OS validattion test, proceeding..."
fi
...
apt-get --allow-unauthenticated install -y bash-completion binutils gpg
...
### install podman
. /etc/os-release
echo "deb http://download.opensuse.org/repositories/devel:/kubic:/libcontainers:/stable/Debian_${VERSION_ID}/ /" | sudo tee /etc/apt/sources.list.d/devel:kubic:libcontainers:testing.list
curl -L "http://download.opensuse.org/repositories/devel:/kubic:/libcontainers:/stable/Debian_${VERSION_ID}/Release.key" | sudo apt-key add -
apt-get update -qq
